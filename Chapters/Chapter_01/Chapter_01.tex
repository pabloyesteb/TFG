...
\end{figure}
%
\indent Beyond global error statistics, the error distribution can be analysed instead. There are two main reasons for doing this:\\
\paragraph{\GMVred{A.} \myul[GMVred]{Non gaussian error distribution}\\}
It is common in industry-applied problems to find situations in which the model's performance leads to non-Gaussian error distributions (see \eg \cite{chen2003non,pernot2020impact,smyl2021learning,chai2019using}). Amongst the reasons for this, the most frequent are, on the one hand, non-homogeneous data sampling in the train set (which leads to uncovered regions and isolated points) which can cause poor model performance due to out-of-interpolation-regime operation, and on the other hand, the inherent difficulty encountered at predicting outputs for specific input configurations due to strongly non-linear physics or governing equations (high gradients). When the error is non-Gaussian, outliers in the error distribution are not represented by concentration statistics such as MAE or RMSE. In such case, a comprehensive analysis over (not only global error measures, but) the whole error distribution is more adequate.\\
%
\paragraph{\GMVred{A.} \myul[GMVred]{Uncertainty measuring}\\}
Statistics of the error's distribution are the first step in the journey towards building an \textbf{uncertainty model}. This is the ultimate milestone of the whole validation pipeline, since it provides precise information about \textit{how much} and \textit{when} the model's predictions are trustworthy. Such model provides confidence intervals inside which the NN can be trusted to predict results inside some error boundary. This is the whole point of \autoref{sec:uncertainty}.\\
%
...
\end{figure}
%
\indent A first indicator of the error distribution over the output variable range can be computing by measuring the goodness of fit of the predicted to the true output distribution, as is illustrated in \autoref{fig:yvsydoublehist}, where ground true values and model's predictions are represented in a double histogram for MS-18. If the model accurately predicts results along the whole output variable range, we would expect both the true and the predicted results to come from the same (unknown) distribution ($H0$). Once again, the null hypothesis is rejected only at a 95\% confidence level. The corresponding p-values of the K-S test are depicted in \autoref{tab:yvsyKS}.\\
\begin{table}
	\centering
...
%
\indent The next step in the error distribution analysis is given in \autoref{tab:errorstats}, where some informative statistics are given. The statistics are computed as plain figures and as confidence intervals using bootstrapping \cite{efron1992bootstrap}. For this last case, the 95\% CI is given for each magnitude.\\
%
\indent Moreover, in \autoref{tab:basicuncertainty} a basic uncertainty model is given in the form of bootstrapped quantiles of the error distribution. If the uncertainty model is trusted (this issue will be addressed in \autoref{sec:uncertainty}, then any prediction whose input features lie inside the range spanned by the test set can be assumed to be true with a maximum uncertainty defined by the quantiles given in \autoref{tab:basicuncertainty}. For illustration, consider for instance that we want to estimate the model's uncertainty with a 95\% CI. In \autoref{tab:basicuncertainty} we can see that the \ordinalnum{5} and the \ordinalnum{95} percentiles of the error distribution for the output variable ''RF Forced Crippling'' correspond to the intervals given by $[-0.0548,-0.0353]$ and $[0.0379.0.0586]$. Then, if (again) the uncertainty model is trusted, we can assume that for new predictions (whose input features are inside the hypercube defined by the test set) 90\% of the error will belong to the interval $[-0.0548, 0.0586]$. Further discussion of this powerful estimation of the error is provided in \autoref{sec:uncertainty}.\\
\begin{sidewaystable}
		\centering
...
\begin{sidewaystable}
		\centering
		\caption{Basic uncertainty model: Bootstrapped percentiles (1, 5, 10, 90, 95 and 99) of the residue distribution, calculated with a 95\% CI using Wilson score \cite{wilson1927probable}.}
...
\end{sidewaystable}
\clearpage
\section{Bias: input space}
%
\clearpage
\section{Bias: output space}
%
\clearpage
