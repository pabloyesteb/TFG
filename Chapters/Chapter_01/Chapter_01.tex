...
\begin{lstlisting}[style=pseudocode]
	For every test point X=(X_{numerical},X_{categorical}) in Test_set:
	Compute Filtered_Training_Set (FTS) by imposing X_{categorical}
	Compute nearest neighbor distance d(X,FTS) and annotate
	Compute average nearest neighbor distance between X_{training} and FTS and annotate
	if (|FTS| not OK):
	annotate and print
	else:
...
\end{lstlisting}
%
The following is a list of specific parameters and requirements needed for the analysis:\\
\begin{itemize}
	\item The code checks if there are enough training samples inside each training subset (\texttt{voxel\_size\_req}).
	\item The code checks if the test point is inside the ranges of the training samples. The percentage of points outside is to be compared with (\texttt{hypercube\_req}).
	...
	\item The code stores all these requirements in a table.
\end{itemize}
%
\begin{table}[h]
	\centering
