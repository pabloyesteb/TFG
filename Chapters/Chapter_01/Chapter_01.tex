...
\begin{theorem}\cite{barany1988shape}
...
%
\indent What's more, the authors in \cite{bonnasse2022interpolation} not only show that interpolation indeed occurs (at least in a latent representation of the ambient space), but that more important conditions to the model's performance are that unseen and training data follows the same cumulative distribution (or the same tail distribution when the former cannot be fulfiled) and that the average distance between training points is similar to the average distance between training and testing points.\\
%
\subsection{Proposed validation loop for the train-test}
A validation loop for the train-test split is given in the form of a flux diagram whose nodes are different tests aimed at checking statistical properties which characterize the split as has been discussed in the previous section. The diagram derives the results of the tests to apropriate recommendations given to the data engineer. Before showing the complete loop, it is convenient to state the nomenclature conventions used in the diagram for the sake of clarity:\\
