...
\end{figure}
%
\section{Distributed error quantification}
This section corresponds to box labelled ''$P(E)$'' in \autoref{fig:pipeline}.\\
%
...
It is common in industry-applied problems to find situations in which the model's performance is not Gaussian at all (see \eg \cite{chen2003non,pernot2020impact,smyl2021learning,chai2019using}). Amongst the reasons for this, the most frequent are non-homogeneous data sampling in the train set (isolated regions, etc.) which can cause poor model performance due to interpolation region issues, and the inherent difficulty of predicting outputs for specific input configurations due to strongly non-linear physics (high gradients). When the error is non-Gaussian, outliers in the error distribution can differ from global error indicators such as MASE or RMSE. In such case, analysing the whole error distribution is needed.\\
%
\paragraph{\GMVred{A.} \myul[GMVred]{Uncertainty measuring}\\}
Measures on the error's distribution are the first step in the journey towards building an uncertainty model. This model provides confidence intervals inside which the NN can be trusted to predict results with an accuracy level defined by some error boundary. This is the main point treated in \autoref{sec:uncertainty}.\\
%
