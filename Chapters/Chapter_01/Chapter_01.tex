...
\end{table}
%
\indent Column data ''inside vox. hypercube'' of \autoref{tab:geo_out} have been rearranged for the sake of interpretability in \autoref{tab:outhypercube}. This table shows, for every voxel in which such points exits, the points which lie outside the training voxel hypercube. Similarly, \autoref{tab:outch} and \autoref{tab:outambient} show points lying inside hypercube but outside the voxel's convex hull in the PCA99 projection, and points lying outside the voxel's convex hull in ambient space but inside the convex hull in the PCA99 projection, respectively.\\
\begin{table}[!htb]
	\centering
...
\begin{table}[!htb]
	\centering
	\captionof{table}{Points outside voxel's PCA99 convex hull but inside hypercube}
	\label{tab:outch}
	\includegraphics[scale=\tabscale]{/home/pablo/Documentos/TFGP/Thesis/Figures/tt-split/outside_CH_c.png}
\end{table}
%
\begin{table}[!htb]
	\centering
	\captionof{table}{Points outside ambient space voxel's convex hull but inside PCA99 convex hull}
	\label{tab:outambient}
	\includegraphics[scale=\tabscale]{/home/pablo/Documentos/TFGP/Thesis/Figures/tt-split/outside_ambient_c.png}
\end{table}
%
\indent The information in \crefrange{tab:outhypercube}{tab:outambient} is used to complete \autoref{tab:reqs_results_table}. The\\
% 
\begin{figure}
	\centering
	\includegraphics[scale=\tabscale]{/home/pablo/Documentos/TFGP/Thesis/Figures/tt-split/coloreqs_c.png}
	\caption{Output messages. Names of the requisites and their descriptions.}
	\label{coloreqs}
\end{figure}
