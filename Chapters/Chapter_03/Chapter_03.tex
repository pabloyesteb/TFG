...
\begin{algorithm}
	...
	\label{algo:tt-split}
	\KwData{${S}^{\text{train}},{S}^{\text{test}}$, voxel\_size\_req}
	...
	\ForEach{$\mathbf{x}=(\mathbf{x_{\text{numerical}},x_{\text{categorical}}})$ in ${S}^{\text{test}}$}{
		Compute test\_voxel by imposing $\mathbf{x_{\text{categorical}}}$\;
		Append test\_voxel to $\text{test\_voxels}$\;
	}
	
	\ForEach{$\mathbf{x}=(\mathbf{_{\text{numerical}},x{\text{categorical}}})$ in ${S}^{\text{train}}$}{
...
The FUM is a conservative uncertainty model that consistently takes, for a given point of the test set and its associated k-th reserve factor $y$, $({\bf x},y)$, the largest uncertainty:
