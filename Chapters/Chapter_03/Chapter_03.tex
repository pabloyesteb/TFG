...
%
	...
	\end{equation}
	%
	\subsection{Yamanaka-Ankersen STM and integration constants.}
	%
...
%		\indent This expression could have also been reached similarly to the in-plane counterpart. 
	...
	%
	\begin{itemize}
	\item[\GMVred{$\bullet$}] \myul[GMVred]{Chief's reference orbit} (Keplerian OEs)
	% 
	\item[\GMVred{$\bullet$}] \myul[GMVred]{Deputy's relative position and velocity with respect to chief} (LVLH frame)
	%
	\item[\GMVred{$\bullet$}] \myul[GMVred]{Propagation values}: Time elapsed and numerical method for high-fidelity propagation(algorithm, timestep \ldots)
	\end{itemize}
	%
	...
	%
	%
		\subsubsection{Scenario 1: $\bm{e = 0.1}$.}
		%
%		%
		...
		%
		\subsubsection{Scenario 2: $\bm{e = 0.7}$.}
		%
		%
...
%
	...
	%
	\paragraph{\textcolor{GMVred}{I.} \myul[GMVred]{Parameter reformulation.} \\}
	%
	...
	%
	\paragraph{\textcolor{GMVred}{II.} \myul[GMVred]{Radial and cross-track coordinates.}\\}
	%
...
%	
	...
	%
		...
		%
		\subsubsection{Eccentricity/inclination vector separation in eccentric orbits.}\label{secCh3:EI}
		%
		...
		%
		\paragraph{\textcolor{GMVred}{I.} \myul[GMVred]{Scenario definition.}\\}
		%
		...
		%
		\paragraph{\textcolor{GMVred}{II. A.} \myul[GMVred]{Bounded trajectory case ($\bm{C_1 = 0}$).} \\}
		%
		...
		%
		\paragraph{\textcolor{GMVred}{II. B.} \myul[GMVred]{Unbounded trajectory case ($\bm{C_1 \neq 0}$).} \\}		
		%
