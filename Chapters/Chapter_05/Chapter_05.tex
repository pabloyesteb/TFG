\chapter{Perturbed relative motion.}
%
\label{chap:Chap_5}
%
...
%
	\subsection{Relative orbit propagation in perturbed relative motion.}
	%
	...
	%
	\input{Chapters/Chapter_05/Mean_prop_workflow}
	\input{Chapters/Chapter_05/Cart2mean_workflow}
	%
\section{Gim-Alfriend STM.}\label{secCh5:GA_STM}
%
	\subsection{Gim-Alfriend STM structure.}
	%
	...
	%
	\begin{itemize}
	\item[\GMVred{\nth{1}}] \myul[GMVred]{LVLH to $\delta \underline{OE}$:} Gim and Alfriend make use of the so-called Geometric Method, which is represented by:
	%
	\begin{equation}
	...
	\end{equation}
	%
	\item[\GMVred{\nth{2}}] \myul[GMVred]{$\delta \underline{OE}$ to $\delta \underline{\overline{OE}}$ (osculating to mean):} Equation \eqref{eqCh4:GA_STM} involves the direct propagation of relative osculating OEs, which turns out to lead to a very much complicated form of the matrix $\phi_e$. This is where Brouwer's theory comes into play: the authors suggest a mapping to and from mean elements as:
	%
	\begin{equation}
	...
	%
	\subsection{Linearised mean to osculating transformation.}
	%
	...
	%
	\begin{itemize}
	\item[] \GMVred{Input:} Osculating elements.
	...
	%
	\input{Chapters/Chapter_05/osc2mean_workflow}
	%
%
\section{State-of-the art perturbed STMs.}
%
...
%
\begin{itemize}
\item[\GMVred{I.}] \myul[GMVred]{Gim-Alfriend STM}: Already described. As it proposes two important changes (the geometric method and the mean elements propagation), two versions of it may be considered, so as to isolate the effect of the coordinate transformations. The first one uses the full STM provided by Gim and Alfriend, while the second one just uses the relative orbital element differences propagation (\ie $\overline{\phi}_e(t, t_0)$) and makes use of the full non-linear Brouwer transformation for the mean-osculating transformations, and a regular transformation between osculating elements and LVLH frame. This one is referred to as the Gim-Alfriend core method.
%
\item[\GMVred{II.}] \myul[GMVred]{Yan-Alfriend STM}: In an alternative approach, Yan and Alfriend \cite{Yan_Alfriend} propose an expansion of the mean orbital element differences up to second-order, leading to a nonlinear model including second-order $J_2$ effects.
%
\item[\GMVred{III.}] \myul[GMVred]{GAM STM}: Developed by Gaias et al. \cite{GAM}, it includes a time-varying differential drag as well as the $J_2$ effect for a more accurate relative motion model. The main setback is the assumption of near-circular reference orbits, which restricts this theory to a limited amount of scenarios
%
\item[\GMVred{IV.}] \myul[GMVred]{KGD STM}: Constructed by Koenig et al. \cite{Koenig}, it uses Floquet theory to arrive to a compact STM. It is developed for several types of OEs (singular, quasi-nonsingular and fully nonsingular) and can be generalized so as to incorporate any type of perturbations.
%
\item[\GMVred{V.}] \myul[GMVred]{Biria-Russell STM}: Biria and Russell chose to incorporate Vinti's theory rather than Brouwer's, which through the usage of spheroidal coordinates allows for a simpler, more precise formulation. This theory is claimed (and proven) to provide a higher accuracy than Brouwer's.
%
\end{itemize} 
%
...
%
	...
	%
	%
		\subsubsection{Cartesian formulation: HCW equations.}
		%
		...
		%
		\subsubsection{Relative OE formulation: GVEs.}
		%
		...
		%
		\subsubsection{Conclusion: HCW solution in terms of orbital elements.}
		%
		...
		%
	\subsection{Effect of non-spherical gravity field: $J_2$.}
	%
...
%	\left\{ \begin{array}{lll}
	...
	\dot{M}^{(s)} 		&= 1 + \dfrac{\alpha}{4} \dfrac{1}{a_0^2 \eta_0^3} \left( 3 \cos^2 i_0 - 1 \right)
 	...
 	\end{subequations}
	%
	\indent The relative growth of the deputy with respect to the chief in E/I elements can be simply obtained by its definition:
	%
	\begin{subequations}
	...
	\delta i_y 	& \approx \delta i_{y0} - \dfrac{\alpha}{4} \dfrac{1}{a_0^2 \eta_0^4} \sin^2 i_0 \delta i_{x0} (\lambda - \lambda_0)
 	...
 	%
 	\paragraph{Effect on the eccentricity and inclination vectors \\} 
 	%
