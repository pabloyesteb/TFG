...
%
\item Right vertical bar subscript: Denotes the coordinate system in which one vector or matrix is described. For example, $\underline{u}\rvert_{1} = \sum_{i=1}^3 u_{xi} \underline{\hat{e}_{i1}} $.
%
\end{itemize}
%
...
%
	...
	\subsection{Direct analytical differentiation.}
	%
...
%
	...
	R_{PQW\to ECI}' & 0_{3x3} \\
	-\Omega_{PQW \wrt ECI}\rvert_{PQW} R_{PQW\to ECI}' & R_{PQW\to ECI}'
	\end{array}\right]
			\]
			%
...
%
	...
	%
		...
		%
		\begin{figure}[!htb]
		\centering\includegraphics[width = 0.4\linewidth]{Appendix_B/RTN_frame}
		...
		\subsubsection{State vector transformation.}
		%
		\indent According to equation \ref{eqAppB: 	MC_direct}
			\paragraph{Rotation matrix from \& to ECI. \\}
			%
			%
			\paragraph{Angular velocity. \\}
